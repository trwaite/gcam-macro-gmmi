  ...
  # For mapping, we will use the naming conventions of the most recent ATB dataset:
    ...
    harmonize_datasets <- function(atb1, atbnext) {
      ...
      atb1 %>%
        ...
        # first year and the overlap year. We assign an arbitrarily high number
        # so we can then replace with NA values
        mutate(ratio = if_else(!year %in% c(first_year,overlap_year),100,ratio)) %>%
        ...
        mutate(ratio = if_else(tech_type == 'Storage' & year <= battery_storage_year,
                               1,ratio)) %>%
        mutate(ratio = dplyr::na_if(ratio,100)) %>%
   ...
   # Establish some year constants for data processing
    ...
    # Gather the ATB Base Years from the the
    ...
    # Now, build past the first ATB year:
    # A23.globaltech_cost_temp %>%
    #   select(-year,-value) %>%
    #   distinct() %>%
    #   repeat_add_columns(tibble(year = energy.ATB_HISTORICAL_YEARS[energy.ATB_HISTORICAL_YEARS > energy.FIRST_ATB_YEAR])) %>%
    #   bind_rows(A23.globaltech_cost_temp) %>%
    #   group_by(technology,input,case) %>%
    #   mutate(value2 = approx_fun(year, value, rule=1)) -> see
    ...
    A23.globaltech_cost %>%
      ...
      # Need to use left_join because the A23 file doesn't have
    ...
    L113.globaltech_cost_atb %>%
      ...
      complete(nesting(technology, input, case, improvement.max, improvement.rate),
               year = A23_years) %>%
      # Copy historical prices back for historicla years:
      group_by(technology,input,case) %>%
      mutate(value = approx_fun(year,value,rule=2)) %>%
