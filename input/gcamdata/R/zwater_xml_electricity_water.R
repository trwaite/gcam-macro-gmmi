...
#' the generated outputs: \code{electricity_water.xml} and \code{electricity_water_alt.xml}. The corresponding file in the
...
module_water_electricity_water_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # TRW 1/18/24: make an alternate version of the xml that has
    # all capital costs of coal and gas CCS moved into the
    # cooling technology level to bypass the issue of gcam-macro only tracking
    # the cooling system capital when these technologies have fixed outputs.

    # get all cooling techs for technologies that will have fixed outputs
    # (elec_coal (conv pul CCS) and elec_gas (CC CCS))
    fixed_output_techs <- c("gas (CC CCS)", "coal (conv pul CCS)")

    cooling_techs <- L2233.GlobalTechCapital_elec_cool %>%
      filter(subsector.name %in% fixed_output_techs) %>%
      select(subsector.name, technology) %>%
      unique()

    # copy non-cooling capital to each cooling technology
    cooling_all_capital <- L2233.GlobalTechCapital_elecPassthru %>%
      filter(technology %in% fixed_output_techs) %>%
      # can't use LJENM because we're adding rows for each cooling tech
      left_join(cooling_techs, by = c("technology" = "subsector.name")) %>%
      select(subsector.name = technology,  year, input.capital,
             capital.overnight, fixed.charge.rate, technology = technology.y) %>%
      mutate(sector.name = paste0("elec_", subsector.name)) %>%
      select(LEVEL2_DATA_NAMES[["GlobalTechCapital"]])

    # remove the non-cooling capital from the pass through techs and add it to
    # the cooling level techs
    L2233.GlobalTechCapital_elecPassthru <- L2233.GlobalTechCapital_elecPassthru %>%
      filter(!technology %in% fixed_output_techs)

    L2233.GlobalTechCapital_elec_cool <- L2233.GlobalTechCapital_elec_cool %>%
      rbind(cooling_all_capital)

    # make the modified xml
    create_xml("electricity_water_alt.xml") %>%
      add_node_equiv_xml("sector") %>%
      add_node_equiv_xml("technology") %>%
      add_logit_tables_xml(L223.Supplysector_elec, "Supplysector") %>%
      add_xml_data(L223.SubsectorShrwtFllt_elec, "SubsectorShrwtFllt") %>%
      ...
      add_precursors("L223.Supplysector_elec",
                     ...
                     "L2233.GlobalIntTechCapital_elec_cool",
                     "L223.GlobalTechCapFac_elec") ->
      electricity_water_alt.xml


    return_data(electricity_water.xml, electricity_water_alt.xml)
  } else {
